...
These 15 reads were previously processed on the SJSU CoS HPC. Quality filtering and primer and adapter removal was performed with Cutadapt. The samples were then imported into QIIME2 for further processing. The 16s libraries were run through DADA2 in QIIME2 to create amplicon sequence variants (ASVs) by denoising and dereplicating paired-end sequences before filtering for chimeras. ASVs were then taxonomically classified with VSEARCH and the SILVA-138 database. A phylogenetic tree was then created by IQ-TREE in QIIME2 for downstream analysis methods that required phylogentic information. 
...
```{r}
sample_data(physeq)[,6:14] <- scale(sample_data(physeq)[,6:14])
```
...
rare_plot <- ggrare(physeq, step = 500, label = sample_names(physeq), 
                    color = "lib_season", parallel = TRUE) +
             geom_vline(xintercept = min(sample_sums(physeq)), 
...
```{r}
env_12_13 <- ggplot(na.omit(subset(palmer_env, lib_season %in% c("12-13"))), 
              aes(x = Date)) +
              ...
              scale_x_date(date_breaks = "months", date_labels = "%b", 
                           ...
                           sec.axis = sec_axis(~., breaks = lib_dates[1:2], 
              ...
              geom_vline(xintercept = as.numeric(lib_dates[1:2]), linetype = 4) +
...
physeq_other <- physeq %>%
  ...
  transform_sample_counts(function(x) {100 * x/sum(x)}) %>%
  psmelt() %>%
  unite(Taxa_Order, Phylum:Class, sep = ";") %>%
...
Weighted Unifrac can be used to calculate the distance between samples. It accounts for the relative abundance of taxa shared between samples and utilizes presence/absence data.
...
physeq_hclust <- hclust(physeq_wunifrac, method="complete")
hclust_dend <- as.dendrogram(physeq_hclust)
mycols <- c(`Early Summer` ="#F8766D", `Mid Summer`="#00BA38", `Late Summer`="#619CFF") 
...
Perform Hellinger transformation on rarefied data. Euclidiean distance is required for linear methods like principal component analysis (PCA) and redundancy analysis (RDA).
...
```{r}
RDA_hel_uncon <- ordinate(physeq_hel,
                    method = "RDA")

RDA_hel_fit <- gg_envfit(RDA_hel_uncon, veg_physeq_hel_sd, 
                         alpha = 1, 
...
p = plot_ordination(
        ...
        ggtitle("PCA") + 
        geom_point(size = 2.5) +
        #geom_text_repel(aes(label = as.character(lib_dates)), size = 3) + 
...
```{r}
fit_RDA <- envfit(RDA_hel_uncon, veg_physeq_hel_sd, permutations = 10000)
fit_RDA
```

Temperature and Nitrite and Nitrate have the highest correlation and lowest P values.
...
About 30% of the variance can be explained by Temperatrue and Nitrite and Nitrate
...
```{r}
physeq_comp <- transform(physeq, "compositional")
```
...
```{r}
summer_stages <- unique(as.character(meta(physeq_comp)$Summer_Stage))

physeq_comp_order <- physeq_comp %>%
tax_glom(taxrank = "Order")

core_summer_stage_order <- c()
...
for (n in summer_stages){
    ...
    core_summer_stage_order[[n]] <- core_m
}
```
...
# arrange plots and add spacing between
...
Core taxa names in Summer Stages
```{r}
...
```{r}
Reduce(intersect, list(early_summer_core_order, 
                       mid_summer_core_order,
                       late_summer_core_order))
```

Core taxa unique to Early Summer and Late Summer only

```{r}
setdiff(intersect(early_summer_core_order, late_summer_core_order), mid_summer_core_order)
```

Core taxa unique to Mid Summer and Late Summer only

```{r}
setdiff(intersect(mid_summer_core_order, late_summer_core_order), early_summer_core_order)
```

Core taxa unique to Early Summer only

```{r}
setdiff(setdiff(early_summer_core_order, late_summer_core_order), mid_summer_core_order)
```

Core taxa unique to Late Summer only

```{r}
setdiff(setdiff(late_summer_core_order, early_summer_core_order), mid_summer_core_order)
```
